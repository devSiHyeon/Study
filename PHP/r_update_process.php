...
// 본문 게시판 번호 가져오기
$sql    = "SELECT detail_idx FROM reply WHERE idx = '$idx'";
$result = mysqli_query($db, $sql);
$row    = mysqli_fetch_assoc($result);
// echo $row['detail_idx'];

// 댓글 내용 수정
$sql_u    = "UPDATE reply SET `content`='$content', `updated_time`= NOW() WHERE idx = '$idx'";
$result_u = mysqli_query($db, $sql_u);

if($result_u){
//    echo "<script>alert('수정 완료 되었습니다.');history.go(-2);</script>";
    echo '<script>location.href=\'./b_view.php?idx='.$row['detail_idx'].'\';</script>';
} else {
