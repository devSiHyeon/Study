<?php require_once ($_SERVER['DOCUMENT_ROOT'].'/pdo/db.php');
    ...
    if (isset($_GET['file_idx'])) {
        ...
        $f_save = '/home/se/public_html/board/'.$row['save'].'/'.$r_name;
        ...
        $sql_3 = $db ->prepare('SELECT file_name, rand_name, save, type from board_file WHERE board_idx = ?');
        ...
        if(!empty($arr)){
            ...
            $f1_rand    = $f1['rand_name'];
            $f1_save    = '/home/se/public_html/board/'.$f1['save'].'/'.$f1_rand; 
            ...
            if (count($arr) >1){
                $f2         = $arr[1];
                print_r($f2_name    = "$f2[file_name]");
                print_r($f2_rand    = "$f2[rand_name]");
                print_r($f2_save    = '/home/se/public_html/board/'.$f2['save'].'/'.$f2_rand); 
        ...
        if ($sql_6->execute(array($idx)) != false) {
            echo '<script>alert(\'파일 삭제 완료\');</script>';
        } else { 
            echo '<script>alert(\'삭제 오류 \n 관리자에게 문의하세요.\')</script>';
            pre($f1_save);
        }
