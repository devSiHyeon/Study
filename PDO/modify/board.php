<?php require_once ('/home/se/public_html/pdo/db.php');
    ...
    foreach($files['error'] as $ikey => $item) {
        ...
        if ($item == 0 &&  1048576 > $size && $size> 0 && in_array($extension, $mime)) {     // 파일 유무, 용량, 타입 확인
            ...
            // 디렉토리 생성
            if (!file_exists('/home/se/public_html/board/'.$extension)) {
                mkdir($extension, 0777, true);             
            } 
            // 파일 upload
            if (true === move_uploaded_file($tmp_name,'/home/se/public_html/board/'.$extension.'/'.$rand_name)) {
    ...
    $getFile = $db -> prepare ('SELECT idx FROM board_file WHERE board_idx = ?');
    $getFile->execute(array($idx));
    $testRow = $getFile -> fetchAll();
        ...
        if(isset($insert)){
            ...
            foreach ($insert as $key => $values) {  
                    ...
                    if ( isset($testRow[$key]) ) {
                        $sql_file = $db -> prepare('UPDATE board_file SET  `file_name` = ?, `rand_name` = ?, `tmp_name` = ?, `save` = ?, `type` = ?, `ip_address` = ?, `upload_time` = NOW() WHERE idx = ?');
                        $sql_file -> execute(array($values['sysn'],$values['rand'],$values['tmp'],'./'.$values['save'],$values['extension'],$ip, $testRow[$key]['idx']));
                        echo 'update';
                    } else {
                        $sql_file = $db->prepare('INSERT INTO board_file (`board_idx`, `file_name`, `rand_name`, `tmp_name`, `save`, `type`, `ip_address`, `upload_time`) 
                            VALUES (?, ?, ?, ?, ?,?,?, NOW())');
                        $sql_file->execute(array($idx ,$values['sysn'], $values['rand'], $values['tmp'], './'.$values['save'], $values['extension'], $ip));
                        echo 'insert';
                    }
    ...
    echo '<script>alert(\'업로드 완료했습니다.\');location.href=\'/pdo/boardlist.php\';</script>' ;
?>