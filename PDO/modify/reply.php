...
$sql = $db->prepare('SELECT reply_idx, user_id, content FROM reply WHERE idx = ?');
...
if($_SESSION['user_id'] != $row['user_id']){
    ...
    ?>
    <h3>댓글 수정</h3>
    <form name = "reply_update" action="../modifyProcess/reply.php" method="POST">
        
        <input type="hidden" name="reply_idx" value="<?=$idx?>">
        아이디 : <label name = "user_id"><?=$_SESSION['user_id']?></label><br>
